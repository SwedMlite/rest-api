# Лабораторна робота №6 — REST API «користувачі / публікації / коментарі»

Це Python‑реалізація лабораторної роботи №6: REST сервіс на FastAPI + SQLAlchemy + MySQL, що надає CRUD та фільтри для трьох сутностей — користувачі, публікації та коментарі. Додаток відповідає вимогам на 5 балів (робота безпосередньо з БД, збереження звʼязків та валідація).
...
2. Імпортуйте схему та дані:
   ...
   ```
   Скрипт створить БД `rest_lab6` з таблицями `users`, `posts`, `comments`, індексами та тестовими записами. Якщо БД вже існує, виконайте `DROP DATABASE rest_lab6;` перед імпортом.
...
Бажаєте додати інші варіанти зберігання (JSON файли чи In‑Memory)? Можна реалізувати альтернативні бекенди поруч у цьому ж репозиторії, але основний варіант на 5 балів — FastAPI + MySQL — уже повністю готовий до захисту.
